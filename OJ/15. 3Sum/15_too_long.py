class Solution:
    ...
    def threeSum(self, nums):
        ...
        if length_even > 2:
            for i in range(length_even):
                ...
                if target_even > even_nums[length_even - 1] + even_nums[length_even - 2]:
                    break
                elif target_even >= even_nums[0] + even_nums[1] and (i == 0 or (i != 0 and even_nums[i] != even_nums[i - 1])):
                    even_even_front = -2
                    even_even_back = -2
                    ...
                    if -1 <= even_even_front <= length_even and -1 <= even_even_back <= length_even:
                        if even_even_back - even_even_front > 2:
                            if target_even != 0 or even_even_back - even_even_front > 3:                        # and ...
                                num1.append(target_even / 2)
                                num2.append(target_even / 2)
                                num3.append(even_nums[i])
                                count += 1
                        elif even_even_back - even_even_front == 1:
                            for j in range(0, even_even_front + 1):
                                if even_nums[even_even_front - j] < target_even - even_nums[length_even - 1]:
                                    break
                                else:
        ...
        if length_odd > 1 and length_even > 0:
            for i in range(length_odd):
                ...
                if target_odd > odd_nums[length_odd - 1] + even_nums[length_even - 1]:
                    break
                elif target_odd >= odd_nums[0] + even_nums[0] and (i == 0 or (i != 0 and odd_nums[i] != odd_nums[i - 1])):
                    target_odd = -odd_nums[i]
                    odd_even_front = -2
                    odd_even_back = -2
                    odd_odd_front = -2
                            ...
                            if even_nums[odd_even_front - j] < target_odd - odd_nums[length_odd - 1]:
                                ...
                                for k in range(odd_odd_back, length_odd):
                                    if odd_nums[k] > target_odd - even_nums[0]:
                                        break
                                    elif odd_nums[k] == target_odd - even_nums[odd_even_front - j] and k != i and (odd_even_front - j == odd_even_front or (odd_even_front - j != odd_even_front and even_nums[odd_even_front - j] != even_nums[odd_even_front - j + 1])) and (k == odd_odd_back or (k != odd_odd_back and (odd_nums[k] != odd_nums[k - 1] or k - 1 == i))):
                    ...
                    if -1 <= odd_odd_front <= length_odd and -1 <= odd_even_back <= length_even:
                        for j in range(0, odd_odd_front + 1):
                            if odd_nums[odd_odd_front - j] < target_odd - even_nums[length_even - 1]:
                                break
                            else:
        ...
        result_unique = []
        if count > 0:
            result_unique.append(result[0])
            for i in range(count - 1):
                if result[i + 1] != result[i]:
                    result_unique.append(result[i + 1])
